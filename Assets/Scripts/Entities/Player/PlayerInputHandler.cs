...
namespace Entities.Player
{
    public class PlayerInputHandler : MonoBehaviour
    {
        ...
        {
            // 방향 입력 처리
            float horizontal = Input.GetAxisRaw("Horizontal"); // A(-1) / D(+1) / None(0)
            float vertical = Input.GetAxisRaw("Vertical");     // W(+1) / S(-1) / None(0)

            movementInput = new Vector2(horizontal, vertical);
            
            // Enter: 처음 움직임 시작
            if (movementInput != Vector2.zero && _lastMovementInput == Vector2.zero)
            {
                OnMoveEnter?.Invoke();
            }
